// controllers/pedidos/authController.js
import db from '../../config/db.js';
import bcrypt from 'bcryptjs';
import jwt from 'jsonwebtoken';

const ROLE_PEDIDOS = 6;
 ...
 * }
 */
export const loginPedidos = async (req, res) => {
  try {
    const rawInput =
      req.body.input ??
      req.body.usuario ??
    ...
    if (user.eliminado === 1) return res.status(403).json({ message: 'La cuenta está eliminada' });
    if (user.activo !== 1)   return res.status(403).json({ message: 'La cuenta está inactiva' });
    if (Number(user.rol_id) !== ROLE_PEDIDOS) {
      return res.status(403).json({ message: 'Este usuario no es de Pedidos' });
    }
    ...
    const token = jwt.sign(
      { uid: user.id, rol_id: user.rol_id, scope: 'pedidos' },
    ...
    console.error('[pedidos/login] error:', e);
    return res.status(500).json({ message: 'Error en el login de Pedidos' });
  }
};
